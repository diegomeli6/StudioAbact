# -*- coding: utf-8 -*-
"""
Genera quiz ed examQuiz accademici per Stull Capitoli 29 - 36.
...
with open("scripts/stull_clean_part4.json", "w", encoding="utf-8") as f:
    json.dump(data_p4, f, indent=2, ensure_ascii=False)

print(f"Salvata tranche capitoli 29-36 puliti con successo ({len(data_p4)} capitoli, {sum(len(v['quiz']) for v in data_p4.values())} quiz, {sum(len(v['examQuiz']) for v in data_p4.values())} examQuiz)")
