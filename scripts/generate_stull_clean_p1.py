# -*- coding: utf-8 -*-
"""
Genera quiz ed examQuiz accademici per Stull Capitoli 1 - 10.
...
with open("scripts/stull_clean_part1.json", "w", encoding="utf-8") as f:
    json.dump(data_p1, f, indent=2, ensure_ascii=False)

print(f"Salvata tranche capitoli 1-10 puliti con successo ({len(data_p1)} capitoli, {sum(len(v['quiz']) for v in data_p1.values())} quiz, {sum(len(v['examQuiz']) for v in data_p1.values())} examQuiz)")
