# -*- coding: utf-8 -*-
"""
Genera quiz ed examQuiz accademici per Stull Capitoli 20 - 28.
...
with open("scripts/stull_clean_part3.json", "w", encoding="utf-8") as f:
    json.dump(data_p3, f, indent=2, ensure_ascii=False)

print(f"Salvata tranche capitoli 20-28 puliti con successo ({len(data_p3)} capitoli, {sum(len(v['quiz']) for v in data_p3.values())} quiz, {sum(len(v['examQuiz']) for v in data_p3.values())} examQuiz)")
