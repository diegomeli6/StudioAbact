# -*- coding: utf-8 -*-
"""
Genera quiz ed examQuiz accademici per Stull Capitoli 11 - 19.
...
with open("scripts/stull_clean_part2.json", "w", encoding="utf-8") as f:
    json.dump(data_p2, f, indent=2, ensure_ascii=False)

print(f"Salvata tranche capitoli 11-19 puliti con successo ({len(data_p2)} capitoli, {sum(len(v['quiz']) for v in data_p2.values())} quiz, {sum(len(v['examQuiz']) for v in data_p2.values())} examQuiz)")
